...
1. What problem does the context API help solve?
   Context API enables one to pass state around a component tree, grandchild components per say without having to prop drill.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
   The redux store is the single source of truth because it is where all state is held and updated, whenever a user create an action in our UI, it calls our actions so it can be dispatched to the reducers, the reducer creates our logic and then the store/state is updated therefore our UI being updated with the action the user does.

3. What is the difference between Application state and Component state? When would be a good time to use one over the other?
   Application/global state is state that is used throughout the app and is more general, component level state is component specific, and not used anywhere else in the application.

4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
   Thunk enables one to use Redux and and call action creators that return a function rather than an object. This helps make asynchronous actions.

5. What is your favorite state management system you've learned and this sprint? Please explain why!
   My favorite is Context API because it is not as cumbersome and difficult to setup as Redux and it has the benefits of not having to use prop drilling.
